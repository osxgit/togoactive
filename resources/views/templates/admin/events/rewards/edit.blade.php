        ...
        <main role="main" class="w-full h-full flex-grow p-3 overflow-auto">
            <div class="float-left w-full max-w-screen-xl">
                ...
                <div class="flex w-full bg-white p-2">
                    ...
                    <a href="{{route('admin.events.rewardsPrice.add',array($id,$rewards->id))}}" class="flex justify-center  items-center py-3 no-underline"  style="text-decoration: none;width:48%"><div ><i class="fa fa-cog mr-2" aria-hidden="true"></i>Pricing</div></a>
                ...
                <form method="POST" id="edit-event-rewards-f_010" name="create-event-seo-f_010" action="{{route('admin.events.rewards.update',array($id,$rewards->id))}}" class="w-full float-left" autocomplete="false" enctype="multipart/form-data">
                    @csrf
                    ...
                    <x-forms.section class="mt-8 rounded-xl">
                        ...
                        <x-slot name="section_content">
                            <div class="float-left w-1/2">
                                <x-forms.toggle id="is_core_item" name="is_core_item" value="{{$rewards->is_core_item}}">
                            ...
                            <div class="float-left w-full">
                                <x-forms.textfield id="name" name="name" placeholder="eg- Event Tee / Event Jersey / Finisher Jersey"  :value="old('name',$rewards->name ?? '' )">
                            ...
                            <div class="float-left w-full">
                                <x-forms.textfield id="sku" name="sku" placeholder="eg. Event-Tee"  :value="old('sku',$rewards->sku ?? '' )">
                            ...
                            <div class="float-left w-full ">
                                <x-forms.textarea id="description" name="description" placeholder="Describe the rewards highlights" class="h-32 description ">
                                    ...
                                    {{old('description',$rewards->description ?? '')}}
                                </x-forms.textarea>
                                <x-forms.validationerror>
                                    <x-slot name="field_id">description</x-slot>
                                    <x-slot name="error_text">description_error</x-slot>
                                </x-forms.validationerror>
                                <span class="float-right" id="title_count"><span id="desccount">{{$rewards!= null ?strlen($rewards->description) : 0 }}/</span>200 characters.</span>

                            </div>

                            <div class="float-left w-full">
                                <x-forms.number id="max_quantity" name="max_quantity" placeholder="Select how many quantities to be purchased" :value="old('sku',$rewards->max_quantity ?? '' )">
                    ...
                    <x-forms.section class="mt-8 rounded-xl">
                        ...
                        <x-slot name="section_content">
                            <div class="float-left w-1/2">
                                <x-forms.toggle id="enable_sizing" name="enable_sizing" value="{{ $rewards != NULL && $rewards->size != '' ? 1:0 }}">
                            ...
                            <div id="event_reward_sizing_section" class="hidden">
                                <div class="float-left w-full ">
                                    <x-forms.textfield id="size" name="size" placeholder="Add reward sizes" class="h-32 size " value="{{old('size',$rewards->size ?? '')}}">
...
{{--                                    </x-forms.textarea>--}}
                                ...
                                <div id="sizing_images-label"  class="" style="width:60%" ondrop="drop(event, this)" ondragover="allowDrop(event)">
                                    ...
                                    <x-forms.image_uploader_edit>
                                        <x-slot name="uploder_title"><b>Upload image</b></x-slot>
                                        <x-slot name="uploder_description"></x-slot>
                                        <x-slot name="field_id">sizing_images</x-slot>
                                        <x-slot name="uploaded_img">{{trim($rewards->sizing_images,'"')}}</x-slot>
                                    </x-forms.image_uploader_edit>
    ...
    <script>
        ...
        $("#edit-event-rewards-f_010").validate({
            rules: {
                images_1:{
                    required: true,
                },
            ...
            canvas.toBlob(function(blob) {
                ...
                reader.onloadend = function() {
                    ...
                    $.ajax({
                        ...
                        data: {'_token':  $('input[name="_token"]').val(), 'image': base64data,'eventId':$('#challengeId').val(),'rewardId':{{$rewards->id}} ,'idd':iddname},
        ...
        //var size = document.querySelector('textarea[name=size]');
